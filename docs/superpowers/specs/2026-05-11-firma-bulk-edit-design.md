...
- **Operator role exception:** when the form's role is `operator`, the Firma input renders read-only (greyed-out background, no editing) and is populated from the operator's existing `department` (edit) or auto-derived from the selected station's `company` (create — but create can never be operator anyway, see §2). For the edit case, changing the Atölye does NOT re-write Firma — Firma reflects the original record. This is consistent with "fullAdmin cannot move an operator across workshops".
...
async def bulk_musteri_companies(
    ...
    async with httpx.AsyncClient(...) as client:
        ...
        for user_id in payload.user_ids:
            try:
                # Fetch user
                resp = await client.get(f"{PB}/api/collections/users/records/{user_id}", headers=headers)
                ...
                patch_resp = await client.patch(
                    f"{PB}/api/collections/users/records/{user_id}",
