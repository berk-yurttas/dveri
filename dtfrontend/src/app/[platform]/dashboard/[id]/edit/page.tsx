...
import { 
  ...
  Wifi, Battery, HardDrive, Smartphone, Plus, X, ArrowLeft
} from "lucide-react"
...
export default function EditDashboardPage() {
    ...
    const loadDashboard = async () => {
      try {
        ...
        if (dashboardData.widgets) {
          const convertedWidgets: PlacedWidget[] = dashboardData.widgets.map((widget, index) => {
            const cellIndex = (widget.position_y || 0) * 6 + (widget.position_x || 0)
            ...
            return {
              ...
              iconName: config.iconName || 'Layout',
          ...
          setPlacedWidgets(convertedWidgets)
        }
      } catch (err: any) {
  ...
  const convertPlacedWidgetsToDashboardFormat = (widgets: PlacedWidget[]): DashboardUpdate['widgets'] => {
  ...
  const handleSuccessModalClose = () => {
    ...
    if (subplatform) {
      router.push(`/${platformCode}/dashboard/${dashboardId}?subplatform=${subplatform}`);
    } else {
      router.push(`/${platformCode}/dashboard/${dashboardId}`);
    }
  }
    ...
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Ekran Düzenle</h1>
            <p className="text-gray-600">"{dashboard?.title}" ekranını düzenleyin</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
            <button
                ...
                className="px-6 py-2 bg-gray-400 hover:bg-gray-600 text-white font-medium rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 transform cursor-pointer"
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50" style={{backdropFilter: 'blur(3px)'}}>
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4 pointer-events-auto shadow-2xl border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-gray-900">Dashboard Güncelle</h2>
              <button
