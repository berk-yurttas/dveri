...
EquipmentTestWidget.config = {
  id: "equipment_test-widget",
  name: "Test Cihazı Ürün Takibi",
  type: "equipment_test",
  color: "bg-teal-500",
  description: "Demirbaş numarasına göre test takibi",
...
interface EquipmentTestWidgetProps {
  widgetId?: string
  dateFrom?: string
  dateTo?: string
}

export function EquipmentTestWidget({ widgetId, dateFrom, dateTo }: EquipmentTestWidgetProps) {
  // Create unique instance identifier
  const instanceRef = useRef(widgetId || `equipment-test-${Math.random().toString(36).substr(2, 9)}`)
  ...
  const getStoredFilters = () => {
    if (typeof window === 'undefined') {
      return { demirbasNo: null, surecAdi: null, surecDurum: null }
    }
    try {
      const stored = localStorage.getItem(`equipment-test-filters-${instanceId}`)
      if (stored) {
  ...
  const [widgetData, setWidgetData] = useState<EquipmentTestData[]>([])
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(`equipment-test-filters-${instanceId}`, JSON.stringify({
        demirbasNo: selectedDemirbasNo,
        surecAdi: selectedSurecAdi,
        surecDurum: selectedSurecDurum
      }))
    }
  }, [selectedDemirbasNo, selectedSurecAdi, selectedSurecDurum, instanceId])
    ...
    const loadOptions = async () => {
      try {
        ...
        if (response.success && response.data && response.data.length > 0) {
          const uniqueDemirbasNo = Array.from(new Set(response.data.map(row => row[2]))).filter(Boolean).sort()
  ...
  useEffect(() => {
    const loadWidgetData = async () => {
      ...
      try {
        ...
        whereClauses.push(`tc.DemirbasNo = '${selectedDemirbasNo}'`)
        ...
        // Query to get equipment test data with pagination
        const response = await api.post<WidgetData>('/reports/preview', {
          sql_query: `
            SELECT
              tg.SurecAdi,
        ...
        if (response.success && response.data && response.data.length > 0) {
          const transformed: EquipmentTestData[] = response.data.map(row => ({
            SurecAdi: row[0] || '',
            SurecDurum: row[1] || '',
            StokNo: row[2] || '',
            Tanim: row[3] || '',
    ...
    loadWidgetData()
  }, [selectedDemirbasNo, selectedSurecAdi, selectedSurecDurum, instanceId, dateFrom, dateTo, currentPage, pageSize, sortColumn, sortDirection, columnFilters])
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <select
              ...
              className="px-1.5 py-0.5 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-teal-500 focus:border-teal-500 outline-none"
            >
              ...
              return (
                <button
                  ...
                  className={`px-1.5 py-0.5 text-xs border border-gray-300 rounded ${
                    currentPage === pageNum ? 'bg-teal-500 text-white' : 'hover:bg-gray-50 bg-white'
                  }`}
                >
                  {pageNum}
                </button>
  ...
  // Show loading state
  if (loading && widgetData.length === 0 && !selectedDemirbasNo) {
    return (
    ...
    <div className="w-full h-full bg-white rounded-lg border border-gray-200 flex flex-col overflow-hidden relative">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-gray-200 flex-shrink-0 relative z-20">
        <div className="flex flex-col">
          <h3 className="text-sm font-semibold text-gray-700">Test Cihazı Ürün Takibi</h3>
          <p className="text-xs text-gray-500 mt-0.5">Demirbaş numarasına göre test takibi</p>
        </div>
              ...
              <div className="absolute z-50 mt-1 w-64 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-auto">
                ...
                {filteredDemirbasNoOptions.map((demirbasNo) => (
                  <div
                    ...
                    className={`px-3 py-2 text-xs hover:bg-teal-50 cursor-pointer ${selectedDemirbasNo === demirbasNo ? 'bg-teal-100 font-medium' : ''}`}
                  >
                    {demirbasNo}
                  </div>
                ))}
      ...
      {!selectedDemirbasNo ? (
        <div className="flex-1 flex items-center justify-center p-4 overflow-hidden">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500">Test cihazı verilerini görüntülemek için demirbaş numarası seçmeniz gerekir</p>
          </div>
        </div>
      ) : (
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="relative" ref={showColumnFilter === column.key ? columnFilterRef : null}>
                          <input
                            ...
                            className="w-full px-1 py-0.5 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-teal-500 focus:border-teal-500 outline-none"
