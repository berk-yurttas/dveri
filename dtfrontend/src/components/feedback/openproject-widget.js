/* eslint-disable */
// OpenProject Feedback Web Component — local copy from openproject-widget/src/widget.js
// Guarded against SSR (document may not exist on server).
...
export class OpenProjectFeedbackWidget extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (this.cancelBtn) this.cancelBtn.addEventListener("click", this.onCancel);
    if (this.closeBtn) this.closeBtn.addEventListener("click", this.onClose);
    if (this.fileInput) this.fileInput.addEventListener("change", this.onFileChange);
    this.syncFromAttributes();
  }

  disconnectedCallback() {
    if (this.form) this.form.removeEventListener("submit", this.onSubmit);
    if (this.cancelBtn) this.cancelBtn.removeEventListener("click", this.onCancel);
    if (this.closeBtn) this.closeBtn.removeEventListener("click", this.onClose);
    if (this.fileInput) this.fileInput.removeEventListener("change", this.onFileChange);
  }
  ...
  syncFromAttributes() {
    ...
    if (subtitle && this.subtitleEl) this.subtitleEl.textContent = subtitle;
    if (defPlatform) this.platformSelect.value = defPlatform;
    const ownerEl = this.shadowRoot.getElementById("owner");
    if (ownerEl) ownerEl.value = defOwner || "Widget User";
    const deptEl = this.shadowRoot.getElementById("department");
    if (deptEl) deptEl.value = defBirim || "Genel";
  }
