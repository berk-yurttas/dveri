...
from app.models.postgres_models import Platform, Dashboard, Report, UserPlatform
    ...
    async def delete_platform(
        ...
        if soft_delete:
            ...
            if dashboard_count.scalar() > 0:
                raise HTTPException(
                    status_code=400,
                    detail=f"Cannot delete platform with existing dashboards. Use soft delete instead."
                )
            ...
            if report_count.scalar() > 0:
                raise HTTPException(
                    status_code=400,
                    detail=f"Cannot delete platform with existing reports. Use soft delete instead."
                )
