...
def check_report_structure(report: Any) -> list[dict[str, Any]]:
    ...
    if color and isinstance(color, str) and not re.match(r"^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$", color.strip()):
        cases.append(case("structure_color", "ui", "Theme color", "passed", f"Color is not a hex value: {color}"))
...
def check_layout(report: Any) -> list[dict[str, Any]]:
    ...
    def _check_one(layout: Any, label: str, case_id: str) -> None:
        ...
        if not isinstance(layout, list):
            cases.append(case(case_id, "ui", label, "passed", "Layout config is not a list"))
            return
        unknown = []
        for item in layout:
...
def check_result_columns(
    ...
    if not columns:
        cases.append(case(
            ...
            "failed" if row_count else "passed",
...
def summarize_cases(cases: list[dict[str, Any]]) -> str:
    failed = [item for item in cases if item.get("status") in {"failed", "error"}]
    if failed:
